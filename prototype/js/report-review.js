  ...
  function openReportDialog(reviewId, user) {
    ...
    wrapper.querySelector("#reportForm").addEventListener("submit", event => {
      ...
      const duplicate = reports.some(report =>
        String(report.reviewId) === String(review.id) &&
        report.reportedBy === user.email &&
        report.status === "Open"
      );
      if (duplicate) {
        toast("You already have an open report for this review");
        return;
      }
      reports.unshift({
      ...
      try {
        ...
        toast("The report could not be saved. Browser storage may be full.");
      }
    });
  }
});
