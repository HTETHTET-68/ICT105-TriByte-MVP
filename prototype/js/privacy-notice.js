document.addEventListener('DOMContentLoaded',()=>{
  ...
  dialog.innerHTML=`
    <div class="privacy-dialog">
      ...
      <ul>
        <li><strong>Account information:</strong> display name, student ID, email address, role, prototype password, account status, profile preferences, and behaviour reports.</li>
        <li><strong>Review information:</strong> category, place name, rating, price, location, review text, optional resized photo, submission time, moderation status, removal requests, helpful votes, and misinformation reports.</li>
        <li><strong>Saved places:</strong> bookmarked dormitories and cafeterias associated with the current prototype account.</li>
      ...
      <p>This version has no real backend. Prototype records—including passwords—are stored only in this browser using <code>localStorage</code>. They are not sent to HallPass servers. Anyone using the same browser profile may be able to view or change the demonstration data. This is not production-grade authentication.</p>
      <h3>Your choices</h3>
      <p>Do not enter real or sensitive personal information. Use fictional demonstration details only. Consent is required to create a prototype account or submit a review. You may remove a pending review, request removal of an approved review, remove saved places, or clear the browser's site data to delete locally stored prototype records.</p>
