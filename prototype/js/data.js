...
const seedReviews = [
  {
    ...
    tag: "Near Campus",
    emoji: "🏢",
    image: "images/reviews/rsu-residence.webp",
    text:
  ...
  },
  {
    ...
    tag: "Budget Friendly",
    emoji: "🏬",
    image: "images/reviews/muang-ake-apartment.webp",
    text:
  ...
  },
  {
    ...
    tag: "Top Rated",
    emoji: "🏙️",
    image: "images/reviews/the-campus-place.webp",
    text:
  ...
  },
  {
    ...
    tag: "Lunch Spot",
    emoji: "🍛",
    image: "images/reviews/rsu-food-court.webp",
    text:
  ...
  },
  {
    ...
    tag: "Halal Friendly",
    emoji: "🍜",
    image: "images/reviews/international-canteen.webp",
    text:
  ...
  },
  {
    ...
    tag: "Study Cafe",
    emoji: "☕",
    image: "images/reviews/cafe-blue.webp",
    text:
  ...
  },
  {
    ...
    tag: "Quiet Area",
    emoji: "🏢",
    image: "images/reviews/lak-hok-garden-residence.webp",
    text:
  ...
  },
  {
    ...
    emoji: "🏬",
    image: "images/reviews/rangsit-campus-view-apartment.webp",
    text:
  ...
  },
  {
    ...
    tag: "Quick Lunch",
    emoji: "🍱",
    image: "images/reviews/building-6-food-court.webp",
    text:
  ...
  },
  {
    ...
    tag: "Study Friendly",
    emoji: "☕",
    image: "images/reviews/green-corner-cafe.webp",
    text:
...
const FAVORITES_STORAGE_KEY = "hallpassFavorites";
const HELPFUL_STORAGE_KEY = "hallpassHelpfulVotes";
const RECENTLY_VIEWED_STORAGE_KEY = "hallpassRecentlyViewed";
...
function getCurrentUserEmail() {
  try {
    ...
    return String(JSON.parse(raw || "null")?.email || "guest")
      .trim()
      .toLowerCase();
  } catch {
    return "guest";
...
function getFavorites() {
  ...
  const values = all[getCurrentUserEmail()];
...
function toggleFavorite(placeId) {
  ...
  all[email] = current.includes(key)
    ? current.filter(item => item !== key)
    : [...current, key];
  localStorage.setItem(FAVORITES_STORAGE_KEY, JSON.stringify(all));
  return all[email].includes(key);
}

function getHelpfulCount(reviewId) {
  const votes = readJsonStorage(HELPFUL_STORAGE_KEY, {});
  return Number(votes[String(reviewId)]?.count || 0);
}

function hasVotedHelpful(reviewId) {
  const votes = readJsonStorage(HELPFUL_STORAGE_KEY, {});
  return Boolean(
    votes[String(reviewId)]?.voters?.includes(getCurrentUserEmail())
  );
}
...
function toggleHelpfulVote(reviewId) {
  ...
  const email = getCurrentUserEmail();
  const votes = readJsonStorage(HELPFUL_STORAGE_KEY, {});
  const record = votes[key] || { count: 0, voters: [] };
  const voted = record.voters.includes(email);
  record.voters = voted
    ? record.voters.filter(item => item !== email)
    : [...record.voters, email];
  record.count = record.voters.length;
  votes[key] = record;
  localStorage.setItem(HELPFUL_STORAGE_KEY, JSON.stringify(votes));
  return { count: record.count, active: !voted };
}

function getVerificationLabel(review) {
  if (review.listingProfile) return "HallPass verified";
  if (review.moderatedAt || review.status === "Approved") {
    return "Admin reviewed";
  }
  return "Student submitted";
}

function getLastUpdatedLabel(review) {
  const value =
    review.moderatedAt || review.submittedAt || "2026-07-28T00:00:00.000Z";
  return new Date(value).toLocaleDateString();
}

function getMapUrl(review) {
  const query = [
    review.title,
    "Rangsit University",
    "Thailand"
...
function recordRecentlyViewed(placeId) {
  const key = String(placeId || "");
  if (!key) return;
  const email = getCurrentUserEmail();
  const all = readJsonStorage(RECENTLY_VIEWED_STORAGE_KEY, {});
  const current = Array.isArray(all[email]) ? all[email] : [];
  all[email] = [key, ...current.filter(item => item !== key)].slice(0, 4);
  localStorage.setItem(RECENTLY_VIEWED_STORAGE_KEY, JSON.stringify(all));
}
